export default async function handler(req, res) {
    ...
    const apiUrl = `https://api.challonge.com/v1/tournaments/${tournamentCode}.json?api_key=${apiKey}`;
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log('Challonge API Response Data:', data); // Check to see if we get all data back
            res.status(200).json(data);
        } else {
            const errorData = await response.text();
            console.error('Challonge API Error:', errorData);
            res.status(response.status).json({ error: 'Failed to fetch tournament data' });
        }
    } catch (error) {
